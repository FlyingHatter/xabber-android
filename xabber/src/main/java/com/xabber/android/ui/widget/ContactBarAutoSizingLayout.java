...
public class ContactBarAutoSizingLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (chat.getVisibility() != GONE) {
            ...
            for (TextView name : textViews) {
                if (name.getText().length() > textSize) {
                    textSize = name.getText().length();
            ...
            if (textViews.get(maxTextIndex).getMeasuredWidth() > (width / 4)) {
                float ratio = (width / 4.1f) / textViews.get(maxTextIndex).getMeasuredWidth();
